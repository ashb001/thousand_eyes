...
public class TweetDao {
	...
	public List<Message> relatedMessagesOf(String currentUser,
	...
	public List<Map<String, String>> mostPopularFollowersReport() {
		...
		return jdbcTemplate.query(
				MOST_POPULAR_FOLLOWER_SQL,
				(MapSqlParameterSource) null,
				(resultSet, i) -> {
					Map<String, String> map = new HashMap<>();
					map.put(resultSet.getString("USER"),
							resultSet.getString("MOST_POPULAR_FOLLOWER"));
					return map;
				});
	...
	@Transactional
	public int startFollowing(String leaderHandle, String followerHandle)
			throws IllegalArgumentException {

		Person leader = findPersonByHandle(leaderHandle);
		if (leader == null) {
			throw new IllegalArgumentException("Cannot unfollow "
					+ leaderHandle + ". Person doesn't exist.");
		}
		...
		return jdbcTemplate.update(INSERT_FOLLOWER_RELATIONSHIP_SQL,
				new MapSqlParameterSource("userId", leader.getId()).addValue(
						"followerId", follower.getId()));
	}

	@Transactional
	public int stopFollowing(String leaderHandle, String followerHandle)
			throws IllegalArgumentException {
	...
	private Person findPersonByHandle(String userHandle) {
		List<Person> people = jdbcTemplate.query(GET_USER_FROM_HANDLE_SQL,
	...
	private final static String INSERT_FOLLOWER_RELATIONSHIP_SQL = "insert into followers (person_id, follower_person_id) values(:userId, :followerId)";

	private final static String DELETE_FOLLOWER_RELATIONSHIP_SQL = "delete from followers where person_id = :userId and follower_person_id = :followerId";

	private final static String GET_USER_FROM_HANDLE_SQL = "select * from people p where p.handle = :userHandle";

	private final static String GET_USER_ID_FROM_HANDLE = "(select id from people where handle = :userHandle)";

	private final static String GET_MESSAGES_SQL = "select * from messages where person_id IN ("
			+ "select person_id from followers where follower_person_id IN "
			+ GET_USER_ID_FROM_HANDLE
			+ " union "
			+ GET_USER_ID_FROM_HANDLE
			+ ") order by person_id";

	private final static String GET_MESSAGES_WITH_KEYWORD_SQL = "select * from messages where person_id IN ("
			+ "select person_id from followers where follower_person_id IN "
			+ GET_USER_ID_FROM_HANDLE
			+ " union "
			+ GET_USER_ID_FROM_HANDLE
	...
	private final static String MOST_POPULAR_FOLLOWER_SQL = "select P.handle as USER, "
